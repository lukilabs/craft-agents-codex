...
const UNEXPECTED_RESPONSE_BODY_MAX_CHARS: usize = 1000;
...
impl UnexpectedResponseError {
    fn display_body(&self) -> String {
        ...
        truncate_with_ellipsis(trimmed_body, UNEXPECTED_RESPONSE_BODY_MAX_CHARS)
    }
...
mod tests {
    ...
    fn unexpected_status_truncates_long_body_with_ellipsis() {
        let long_body = "x".repeat(UNEXPECTED_RESPONSE_BODY_MAX_CHARS + 10);
        let err = UnexpectedResponseError {
        ...
        let expected_body = format!("{}...", "x".repeat(UNEXPECTED_RESPONSE_BODY_MAX_CHARS));
        assert_eq!(
            err.to_string(),
            format!(
